...
export function formatFixPrompt(
  ...
  const header = [
    ...
    "",
    "**Test-harness conventions** (matters for both the JSON block and the in-place edit):",
    "- The harness evaluates test code in a sandbox with `describe`, `it`, `test`, `expect`, `chain`, plus the viem helpers (`parseEther`, `keccak256`, `encodePacked`, etc.) injected as globals. **Static `import` statements at the top of `tests` will fail with `Unexpected token '{'. import call expects one or two arguments.`** — drop them.",
    "- `chain.deploy({ contract: 'Name', args: [...], value: bigint })` is the supported deploy form (positional `chain.deploy('Name', [...])` is not).",
    "- Contract names in `tests` must match contracts emitted by the `solution` / `solutionFiles` source — solc reports them under their declared `contract Foo` name. Mismatches surface as `Contract \"Foo\" not in compiled output. Available: ...` errors.",
    "",
    "---",
  ]
